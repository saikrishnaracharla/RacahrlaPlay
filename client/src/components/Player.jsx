...
import { Play, Pause, SkipBack, SkipForward, Volume2, VolumeX, Repeat, Shuffle, Music, Heart, ListMusic, ChevronUp } from 'lucide-react';
...
export default function Player() {
  ...
  const Art = ({ size = 48 }) => (
    <div style={{ width:`${size}px`, height:`${size}px`, borderRadius:'9px', overflow:'hidden', flexShrink:0, background:'var(--bg-card)', boxShadow: isPlaying ? '0 0 0 2px var(--green)' : 'none', transition:'box-shadow 0.3s' }}>
      {!imgError && currentSong.image
        ...
        : <div style={{ width:'100%', height:'100%', background:'linear-gradient(135deg,#1a1a2e,#16213e)', display:'flex', alignItems:'center', justifyContent:'center' }}><Music size={size/2.5} color="var(--green)" /></div>
      }
    </div>
  );
  ...
  return (
    <>
      ...
      {showFullscreen && <MobileFullPlayer onClose={() => setShowFullscreen(false)} />}
      ...
      <div className="player-bar" style={{ height:'var(--player-h)' }}>
        ...
        {/* ── MOBILE COMPACT — tap to open fullscreen ── */}
        <div className="player-mobile-compact" onClick={() => setShowFullscreen(true)} style={{ cursor: 'pointer' }}>
          {/* Art + info */}
          ...
          {/* Mobile controls — stop propagation so tap on button doesn't open fullscreen */}
          <div style={{ display:'flex', alignItems:'center', gap:'4px', flexShrink:0 }} onClick={e => e.stopPropagation()}>
            <button onClick={e => { e.stopPropagation(); playPrev(); }} style={{ background:'none', border:'none', cursor:'pointer', color:'var(--text-secondary)', display:'flex', padding:'6px' }}>
              <SkipBack size={20} />
            </button>
            <button onClick={e => { e.stopPropagation(); togglePlay(); }} style={{ width:'40px', height:'40px', borderRadius:'50%', background:'var(--green)', border:'none', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', boxShadow:'0 4px 12px rgba(29,185,84,0.4)' }}>
              {isPlaying ? <Pause size={18} color="#000" fill="#000"/> : <Play size={18} color="#000" fill="#000" style={{ marginLeft:'2px' }}/>}
            </button>
            <button onClick={e => { e.stopPropagation(); playNext(); }} style={{ background:'none', border:'none', cursor:'pointer', color:'var(--text-secondary)', display:'flex', padding:'6px' }}>
              <SkipForward size={20} />
            </button>
            {/* Expand indicator */}
            <button onClick={() => setShowFullscreen(true)} style={{ background:'none', border:'none', cursor:'pointer', color:'var(--text-muted)', display:'flex', padding:'6px' }}>
              <ChevronUp size={18} />
            </button>
          </div>
