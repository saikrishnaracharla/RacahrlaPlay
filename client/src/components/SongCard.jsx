...
export default function SongCard({ song, songs = [], index = 0, layout = 'grid' }) {
  ...
  const Thumb = ({ size = 44 }) => (
    <div style={{ width: size, height: size, borderRadius: size > 44 ? '10px' : '6px', overflow: 'hidden', flexShrink: 0, background: '#0C0018' }}>
      {imgSrc
        ? <img src={imgSrc} alt={song.title} onError={() => setImgError(true)} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
        : <div style={{ width: '100%', height: '100%', background: 'linear-gradient(135deg,#0C0018,#1A0030)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <Music size={size / 2.5} color={VG} />
          </div>
      }
    </div>
  );
  ...
  if (layout === 'list') {
    return (
      <div onClick={handleCardClick} className={`song-card ${isActive ? 'song-active' : ''}`}
        ...
        <div style={{ width: '22px', textAlign: 'center', color: isActive ? V : 'var(--text-muted)', fontSize: '12px', flexShrink: 0 }}>
          {isActive && isPlaying
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <p style={{ color: isActive ? '#A78BFA' : '#F0EAFF', fontSize: '14px', fontWeight: '500', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{song.title}</p>
    ...
    <div className={`song-card ${isActive ? 'song-active' : ''}`} onClick={handleCardClick}>
      ...
      <div style={{ position: 'relative', paddingBottom: '100%', overflow: 'hidden' }}>
        {imgSrc
          ...
          : <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(135deg,#0C0018,#1A0030)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <Music size={40} color={VG} />
            </div>
        }
        ...
        {isActive && (
          <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '8px' }}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              <button onClick={handlePlayPause} style={{ width: '42px', height: '42px', borderRadius: '50%', background: `linear-gradient(135deg,${V},#6D28D9)`, border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: `0 4px 16px rgba(139,92,246,0.6)` }}>
        ...
        {!isActive && (
          <button onClick={e => { e.stopPropagation(); playSong(song, songs, index); }}
            ...
            style={{ position: 'absolute', bottom: '8px', right: '8px', width: '42px', height: '42px', borderRadius: '50%', background: `linear-gradient(135deg,${V},#6D28D9)`, border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: `0 4px 16px rgba(139,92,246,0.55)`, opacity: 0, transition: 'opacity 0.2s,transform 0.2s', transform: 'translateY(6px)' }}
      ...
      <div style={{ padding: '10px 12px 12px' }}>
        <p style={{ color: isActive ? '#A78BFA' : '#F0EAFF', fontSize: '13px', fontWeight: '600', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginBottom: '3px' }} title={song.title}>{song.title}</p>
